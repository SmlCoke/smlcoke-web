...
> 提示：最高次项对齐
 ...
 以一道例题为例：$\frac{d^2}{dt^2}r(t)+3\frac{d}{dt}r(t)+2r(t)=t^3+t-3  \quad(2) $
...
**(4) 基本结论四**：当非齐次项为$P_{m}(t)e^{\lambda t}$时，并且$\lambda$为方程的$k$重特征根时(若不是特征根，则$k=0$)，特解应该设为：$t^kQ_m(t)e^{\lambda t}$, 其中$Q_m(t)$是关于$t$的$m$ 次多项式：$b_mt^m+b_{m-1}t^{m-1}+...+b_1t+b_0$。
