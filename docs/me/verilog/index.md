...
**在一个模块中**
wire可以被定义为输入和输出，reg只能被定义为输出， 不能被定义为输入
原因：**reg的改变受一定条件控制，例如时钟**，不能直接对reg类型变量赋值

**模块实例化时**
内部的输入端口只能是wire类型，可以连接外部的wire或者reg ，**因为reg和wire都能驱动wire**，无论外部是wire还是reg，其值一旦改变，wire就会跟着改变。（但是常常用reg类型，因为常常涉及到）
...
### 1.5 值
![](Study_notes_material/Verilog基础/值.webp)
...
**当驱动源冲突时**
![](Study_notes_material/Verilog基础/驱动源冲突.webp)
(1) 驱动源相同，则结果等于驱动源。
(2) 驱动源不同，且均不为z，则结果为z。
...
### 1.7 寄存器类型
寄存器类型(register)分为5种子类型：reg类型、integer类型、real类型、time类型（testbench中介绍）、realtime类型（testbench中介绍）
...
**当位宽不一致时**
![](Study_notes_material/Verilog基础/位宽不一致.webp)
==只需要记住，赋值时的常量为32b（计算机数据宽度）==

### 1.9 存储器语法
![](Study_notes_material/Verilog基础/存储器.webp)

### 1.10 变量的符号
![](Study_notes_material/Verilog基础/变量符号.webp)
...
或者从最低位到最高位运算
或者参见：
![](Study_notes_material/Verilog基础/归约运算示意图.webp)
...
##### ⑧ 条件运算符
![](Study_notes_material/Verilog基础/条件运算符.webp)

##### ⑨ 操作符优先级
![](Study_notes_material/Verilog基础/操作符优先级.webp)
...
![](Study_notes_material/Verilog基础/case.webp)
**在casez语句中，==? 和z== 被当作无关值。**
![](Study_notes_material/Verilog基础/casez.webp)
**在casex语句中，==?，z 和x== 被当作无关值。**
![](Study_notes_material/Verilog基础/casex.webp)
...
![](Study_notes_material/Verilog基础/display.webp)
![](Study_notes_material/Verilog基础/monitor.webp)
![](Study_notes_material/Verilog基础/monitor举例.webp)
...
一般来说next state有三种初始值:**2'bx、2'b0、其他state**。  
...
### 5.6 考虑输出的状态设计
##### 常见输出方式
![](Study_notes_material/状态机设计//考虑输出的状态设计.webp)


### 5.7 三段式经典状态机设计（有输出）
大致模板与二段式相同，只不过由于有输出信号，所以除了==计算新状态的组合逻辑块==和==转移状态的时序逻辑块==之外，还有一个**输出的块**
...
![](Study_notes_material/状态机设计//状态编码输出.webp)
```verilog
...
示例：
![](Study_notes_material/文件IO/文件读操作示例.webp)
...
![](Study_notes_material/文件IO/文件格式示例.webp)
...
**文件输出举例**
![](Study_notes_material/文件IO/文件输出举例.webp)
**读写任务函数**
![](Study_notes_material/文件IO/读写任务函数.webp)
...
![](Study_notes_material/任务与函数/任务举例2.webp)
...
1. ==函数与任务的全部行为都可以用module代替==, **抽象层次**：module>过程块>task>function>if, repeat, for...
2. ==在任务和函数中不能声明线网类型==
3. 任务和函数必须在module内调用
4. 所有输出/输出都是局部寄存器
...
2. 在设计中，使用==一个时钟沿==工作：
![](Study_notes_material/常见问题/时钟沿.webp)
...
6. 常量
![](Study_notes_material/常见问题/常量.webp)