...
基于上海交通大学贺光辉教授课件整理，衔接第1章内容，适配具备数字信号处理、概率论、数模集成电路设计基础的学习场景，核心聚焦**基带传输原理、最佳接收、码间串扰抑制**三大核心模块。
...
!!! question 为什么选择 $t=T$ 时刻作为判决时刻？
    可以把发送信号想象成**水龙头开闸放水，放了整整 $T$ 秒钟**。而接收端的匹配滤波器，就是一个**用来接水的水桶**。

    **如果在 $t < T$ 时刻抽样（比如 $t=0.5T$）**：
    水龙头才开了一半，水还没放完。你这个时候就跑去看桶里有多少水，显然你只收集到了信号的**部分能量**，没有把信号的潜力榨干。

    **如果在 $t = T$ 时刻抽样**：
    水龙头**刚刚好**关上的那一瞬间！此时，属于这个码元的所有“有用能量”已经全部流进了桶里，一滴都没浪费。**此时累积的信号能量达到绝对的最大值。**

    **如果在 $t > T$ 时刻抽样（比如 $t=1.5T$）**：
    水早就停了，你多等了0.5秒。这0.5秒里，桶里不会增加任何有用的水（信号），但天空中一直在下着灰尘（信道里的高斯白噪声）。你等得越久，混入的噪声就越多，导致**信噪比反而下降**。甚至，此时下一个水龙头（下一个码元）可能已经打开，脏水（码间串扰）也流进来了。
...
#### 5.2.2 典型场景门限
1.  **二进制双极性基带信号**：$s(t)=\begin{cases}+a+n(t), & 发1 \\ -a+n(t), & 发0\end{cases}$
    最佳判决门限：
    $$\gamma_0 = \frac{\sigma_n^2}{2a} ln\left(\frac{P(S_2)}{P(S_1)}\right)$$
    - 先验等概时（$P(S_1)=P(S_2)=0.5$），$\gamma_0=0$，是最常用的判决门限
2.  **二进制单极性基带信号**：先验等概时，最佳门限$\gamma_0=\frac{a_1+a_2}{2}$（$a_1$为1的幅值，$a_2$为0的幅值）
...
**先验等概二进制双极性信号（AWGN信道）**：
$$P_B = Q\left(\frac{a}{\sigma_n}\right) = Q\left(\sqrt{\frac{2E_b}{N_0}}\right)$$
- $Q(x)$为互补误差函数，单调递减，$x$越大，误码率越低
- 物理意义：$E_b/N_0$每提升3dB，误码率呈数量级下降
...
- 定量表达式：
  接收信号抽样值（$t=kT_B$）：
  $$y(kT_B) = a_k h(0) + \sum_{\substack{n=-\infty \\ n≠k}}^{\infty} a_n h[(k-n)T_B] + n_R(kT_B)$$
  - 第一项：当前码元的有用信号
  - 第二项：其他码元对当前码元的串扰，即ISI
  - 第三项：噪声采样值
...
2.  求解方式：由输入信号的抽样值构成托普利兹矩阵，求解线性方程组得到抽头系数$c_i$
3.  工程特性：
