...
$$E_1 + E_2 = (e_1 + Bias) + (e_2 + Bias) = e_1 + e_2 + 2 \times Bias$$

**结果多出了一个 Bias！**
因此，浮点乘法计算指数的公式为：

$$\mathbf{E_{result} = E_1 + E_2 - Bias}$$
...
## 八、浮点运算的硬件精度：GRS 位 (Guard, Round, Sticky)
...
1.  **Guard Bit (保护位)**：位于最低有效位（LSB）右侧的第 1 位。用于保存移出时的最高位精度。
2.  **Round Bit (舍入位)**：位于 Guard 右侧的第 2 位。
3.  **Sticky Bit (粘滞位)**：位于 Round 右侧。**关键特性**：只要在移位过程中，有**任何一个非零的位 (1)** 被移过了 Round 位，Sticky 位就会被**“粘”成 1**，且一直保持为 1。它用于判断后续的数是否正好是一半（Tie），以此决定“向偶数舍入”的走向。
