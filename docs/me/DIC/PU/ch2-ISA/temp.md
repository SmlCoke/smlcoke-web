# 计算机组成与设计 第2章 指令：计算机的语言 RISC-V核心笔记
对应课程：《通用及图形处理器架构与系统》
核心内容：指令集体系结构(ISA)基础、RISC-V指令集规范、指令编码、汇编与高级语言映射
...
1.  **简洁源于规整性（Simplicity favors regularity）**
...
3.  **让常见场景更快（Make the common case fast）**
    程序中小常数的使用非常普遍，指令内置立即数操作数，避免额外的内存load指令，大幅提升常用操作的执行效率。
4.  **优秀的设计需要合理的折中（Good design demands good compromises）**
...
- RV32I包含**32个32位通用寄存器**，编号x0~x31，32位数据称为1个**字（word）**
- 核心特性：**x0寄存器硬编码为常数0，无法被改写**，是简化指令设计的关键
- 汇编规范：x0~x31为寄存器编号，也可使用ABI别名（如t0~t6为临时寄存器，s0~s11为保存寄存器）

**寄存器操作示例**
已知f~j分别对应寄存器x19~x23，上述C代码的精确汇编：
```riscv
...
- 字节序：RV32I默认采用**小端序（Little Endian）**，即最低有效字节存放在最低地址
...
| 12位             | 5位 | 3位    | 5位| 7位    |
- 核心opcode：
  - 立即数算术：`0010011`（`addi/xori/ori/andi`等）
...
### 6.2 核心位操作用途
- **移位操作**：sll左移i位等价于乘以2^i；srl无符号右移i位等价于除以2^i
...
| 无条件跳转 | jal rd, label | 跳转至label，将PC+4存入rd（函数调用） |
| 无条件跳转 | jalr rd, offset(rs1) | 跳转至rs1+offset，将PC+4存入rd（寄存器间接跳转） |

### 7.2 比较辅助指令slt系列
用于实现复杂条件分支，先比较结果写入寄存器，再配合分支指令跳转：
...
### 7.5 寻址范围
- 条件分支（SB型）：PC相对偏移±4KiB，适合短距离前后跳转
- 无条件跳转（UJ型）：PC相对偏移±1MiB，适合函数调用
- 寄存器间接跳转（jalr）：可跳转到32位地址空间的任意位置，用于函数返回、函数指针调用
- 超出偏移范围时，汇编器会自动将短跳转转换为长跳转序列

---

## 八、核心知识点总结
1.  **ISA是软硬件的核心接口**，上层软件编译为ISA指令，下层硬件基于ISA实现微体系结构，是计算机架构的核心基石。
2.  **RISC-V的核心优势是开源、规整、可扩展**，固定32位指令长度，6种指令格式字段高度对齐，大幅简化硬件实现。
3.  **四大设计原则贯穿始终**：规整性、小而快、常见场景优先、合理折中，是理解RISC-V设计的核心。
4.  **性能优化核心**：寄存器访问远快于内存，编译器优先将高频变量放入寄存器，仅低频数据放入内存。
5.  **控制流实现**：通过PC相对寻址的分支/跳转指令实现条件判断、循环、函数调用，是程序逻辑的核心支撑。