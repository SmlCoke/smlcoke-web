...
!!! note "aider与`.env`"
    当你进入文件夹并输入 aider 时，发生了以下动作：
    1. **启动程序**：Python 运行 Aider 脚本。
    2. **寻找配置**：Aider 程序的代码里写了一行逻辑：“请检查当前运行目录（Current Working Directory）下有没有一个3. 叫 .env 的文件”。
    3. **加载内存**：如果找到了，Aider 会把文件里的内容读取出来，并临时塞进它自己运行时的“内存环境变量”中。
    4. **生效**：接下来的执行过程中，Aider 就会用这些加载进来的值去连接 Deno 和 OpenRouter。

    如果你同时在多个地方设置了配置，Aider 的听从顺序是：
    1. **命令行参数（最高）**：例如 aider --model ...。
    2. **环境变量**：你在 PowerShell 里手动输入的 $env:AIDER_MODEL = ...。
    3. **当前目录的 .env 文件**。
    4. **Aider 的全局配置文件（通常在用户目录下的 .aider.conf.yml**）。