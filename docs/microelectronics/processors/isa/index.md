...
其他指令编码格式见：[附录](appendix/index.md)
...
!!! question "为什么不是4字节对齐？"
    B-type 和 jal 指令在寻址时都只限定 offset 与 1 个 0 凭借，这样只保证最低位为0。而 32 为指令**需要最低两位都为0**，因此实际上，汇编程序中的 offset[1] 其实永远为0。也就是说，能编码的地址只有全部编码空间的一半！
    那为什么 RISC-V 不涉及为 offset 拼接两个 0 呢？这样可以直接实现地址低两位为0。
    **核心原因：支持“C”标准压缩指令扩展（RVC）**
    RVC 允许将最常用的 32 位指令压缩为 16 位（2 字节） 的指令格式。在支持 RVC 的处理器中，32 位指令和 16 位指令是可以自由混合排放的。**因此只需要保证最低位为0即可**。
    那我如果不实现 C 扩展，做一个纯 RV32I 的 CPU，**能不能把格式改成 {offset, 00} 呢**？
    RISC-V 团队在设计时明确否决了这种做法，原因是为了保持 硬件译码器（Decoder）的统一性与兼容性：
    
    - 如果纯 RV32I 使用 {offset, 00} 译码，而实现 RV32IC（带压缩扩展）时使用 {offset, 0} 译码，那么硬件工程师在设计译码器时，就需要根据是否开启 RVC 来设计两套不同的立即数重构逻辑，编译器也必须针对两种模式编译出不同的二进制。
    - 为了实现一劳永逸，RISC-V 规范规定：无论你当前的硬件实不实现 C 扩展，B-type 和 J-type 的立即数在硬件译码时，一律按照 {offset, 0}（2 字节对齐）来解析。
...
当然，现代更加智能的编译器通常会对数组索引进行优化，自动转换为指针运算以提升性能，但理解底层原理有助于编写更高效的代码。
...
> 约定这些是ABI的工作，目的是让不同二进制程序能够协同工作
> 比如我们强行要求 callee 如果要用 saved reg 就必须保存，这样无论 caller 是当前程序还是外部程序，在编写的时候无需手动将 save reg 压栈，<span style="color:green">caller 可以随便用这些寄存器，反正 callee 会自动保存</span>，但是对于 temp 系寄存器就要格外小心。又比如约定 参数/返回值寄存器，是为了确保不同程序调用这个子过程时可以找到正确的地方存放参数
...
| 静态数据段 | Static Data Segment | 存放**全局变量**、静态变量、常量数组与**字符串** | 生命周期贯穿程序全程，`$gp`(全局指针)初始化为该段基地址`0x10008000`，支持±偏移快速访问 |
| 动态数据段（堆） | Dynamic Data (Heap) | 存放**动态分配的内存** | 由`malloc()/free()`（C）或`new/delete`（Java）管理，从低地址向高地址增长，常见问题包括内存泄漏、悬空指针（Dangling pointer） |
...
3. **二进制兼容性**：标准化的ISA使得编译后的程序可以在**所有兼容该ISA的计算机上运行**，无需重新编译
...
- **输入**：汇编语言文件(.s/.asm)
- **输出**：**可重定位目标模块**(.o/.obj)
- **核心工作**：将**汇编指令翻译为二进制机器码**，同时生成**辅助信息用于后续链接**；release模式下会**移除debug信息与label标签**，生成的汇编代码无符号信息。
...
1. **编译阶段**：Java源码被编译为**平台无关**的**Java字节码（Bytecode）**，存储在Class文件中，字节码是JVM的标准指令集
2. **运行阶段**：JVM加载Class文件，采用两种方式执行字节码：
    - **解释执行**：逐条将**字节码翻译为本地机器指令执行，启动快、执行效率低**
    - **即时编译（JIT, Just In Time）**：识别高频执行的“热方法”，将其**字节码一次性编译为本地机器码并缓存**，后续**直接执行编译后的机器码**，大幅提升重复执行的性能

### 16.2 核心特性

- **可移植性**：一次编译，到处运行。字节码不依赖特定硬件ISA，只要目标平台有兼容的JVM即可运行，无需重新编译
- **性能权衡**：JIT编译**弥补了解释执行的性能劣势**，但需要**额外的内存缓存**编译后的代码，启动速度慢于本地编译的C程序
...
<a class="note-card blue" href="appendix/">
  <span class="card-icon" aria-hidden="true">📎</span>
  <div class="card-title">指令集附录</div>
  <p class="card-desc">指令集架构相关补充内容。</p>
</a>
